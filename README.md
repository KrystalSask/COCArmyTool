...
识别在本机执行。web 端确认候选进入编辑器时自动回传识别样本（截图原图（超限时按原分辨率转码）、机器候选与人工修正结果，含跳过确认的未完成样本）；“一键确认全部”且容器条件通过时额外附带配兵链接。仅用于改进识别模型；桌面版（Tauri）不含任何数据收集。回传直接写入作者的腾讯云 COS。当前承诺范围和限制见[截图识别说明](docs/screenshot-recognition.md)。
