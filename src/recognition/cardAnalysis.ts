...
const recognizeArmySlot = async (
  ...
  if (recognitionSettings.cardClassifier === 'onnx') {
    try {
      ...
      classification = { candidates, source: 'onnx', modelVersion: 'army-card-classifier-cn-v2', preprocessingVersion: 'army-card-left-pad-rgb-chw-div255-v1' }
    } catch (error) {
...
const trimTrailingFalseSlots = (slots: DetectedCardSlot[], region: Exclude<RecognitionRegionKind, 'heroes'>) => {
  ...
  // must never add or remove a slot; this hook remains for structural-only
  // cleanup once the detector exposes explicit false-tail evidence.
  void region
  return slots
}
...
const scorePanelCardStructure = (image: CanvasImageSource, width: number, height: number, panel: NormalizedRect, preflight: ScreenshotPreflight) => {
  ...
  if (!definition) return { cardStructureScore: 0, consistencyScore: 0 }
...
export const analyzeCardLayout = async (file: File, preflight: ScreenshotPreflight): Promise<ScreenshotVisualAnalysis> => {
  ...
  const scoredCandidates = manualLocked ? [] : uniqueCandidates.map((candidate) => {
    ...
    const totalScore = candidate.geometryScore * .22 + structure.cardStructureScore * .58 + heroEvidence * .10 + structure.consistencyScore * .10
  ...
  const regions = await Promise.all(armyRegions.map(async (region) => {
    ...
    const rankedSlots = trimTrailingFalseSlots(await Promise.all(detectCardSlots(pixels, region.rect, { trimTrailingFrameless: regionKind === 'castleArmy' })
      .map((slot) => recognizeArmySlot(canvas, pixels, slot, regionKind))), regionKind)
  ...
  const heroes = analyzeHeroSubcards(canvas, analysisPreflight).map((hero) => ({
    ...hero,
    equipment: hero.equipment.map((item) => ({ ...item, rect: projectRectToPanel(item.rect, selectedPanel) })),
  ...
  return { regions, heroes, selectedPanel, panelCandidates: reportedCandidates }
}
